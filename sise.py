...
for code in set(sise_data.code):
    ...
    for i in range(len(code_sise_data)-2, 0, -1):   # 전날 종가대비 오늘 종가 증가율(-4% or 4%)
        ...
        rate = close_pre / diff_day                 # 증가율
        if rate >= 4:
            # code date rate 전날종가 오늘종가 volume
            result.append([code, code_sise_data[i][1], rate, code_sise_data[i-1][2], code_sise_data[i][2], code_sise_data[i][7]])
